...
- AI
date: 2026-01-18
description: What if reasoning traces could learn their own usefulness? A simple RL
  framing for trace memory, and why one reward signal is enough.
draft: false
linked_project:
- the-policy
related_posts:
...
I'd start with equal split. It's wrong, but it's simple. And simplicity matters, as we'll see.
...
Every time we add a hand-crafted reward, we're encoding human judgment about what makes a good trace. That judgment might be wrong. It definitely won't scale. And it creates a maintenance burden, since someone has to tune the weights between "correctness reward" and "efficiency reward" and "elegance reward."
...
The standard approach to trace quality is categorical: this trace is "good" (store it) or "bad" (discard it). Value functions replace categories with gradients. A trace isn't good or bad. It has a *value*, a number that gets updated with experience.

This changes what the memory learns. **Without values**: memory accumulates traces that were correct when created. **With values**: memory accumulates traces that *consistently help produce correct answers when retrieved*. These aren't the same thing. A trace might be correct but useless as a retrieved example (too specific, wrong abstraction level). A trace might even be incorrect but useful (shows a pattern that triggers good reasoning in the model).
...
The alpha parameter controls the tradeoff. High alpha: trust similarity. Low alpha: trust value.

You could also do something UCB-style, retrieving traces with high uncertainty (low visit count) to explore. But that adds complexity. Start without it.
...
In [The Policy](/post/2024-09-10-the-policy/), SIGMA doesn't cache a policy. At decision time, it searches through possibility space guided by learned Q-values. The Q-function encodes: "in state s, taking action a tends to lead to reward r." My value function encodes: "retrieving trace t tends to lead to correct outputs."

Same structure. SIGMA's Q-values are implicit in neural weights. Trace values are explicit in the memory store. The feedback loop is the same too: SIGMA's learned values shape its decisions, which shape its future learning. Trace values shape retrieval, which shapes what traces get updated.
...
In [All Induction Is the Same Induction](/post/2024-09-30-universal-bayes/), I parameterized learning by four knobs: **P** (hypothesis space), **C(p)** (complexity penalty / prior), **lambda** (noise tolerance), **l** (loss function). Value-weighted retrieval adds a fifth: **utility weighting**.

Standard Bayesian inference weights hypotheses by posterior probability. Decision-theoretic inference weights by expected utility, probability times value. Retrieving by similarity alone is like weighting by probability ("how likely is this trace to be relevant?"). Retrieving by similarity times value is like weighting by utility ("how useful is this trace for getting reward?").
...
What if credit could flow further back? Trace A helped generate Trace B, which helped generate Trace C, which produced a correct output. Should Trace A get credit as a grandparent of the successful solution?

The Bellman equation handles this through gamma: $V(A) \leftarrow V(A) + \alpha \gamma^2 r$. But this requires tracking ancestry (which traces were retrieved when generating each stored trace), which adds storage overhead, potential cycles, and computational cost for propagating updates through the graph.
...
**Discount factor**: What's the right gamma? High gamma means more credit to ancestors. Low gamma means only recent traces matter.
...
The core idea: reasoning traces should have values that learn from experience. Not just "was this trace correct when created?" but "does this trace help produce correct answers when retrieved?"

One reward signal. Let the values learn what matters. This is the bitter lesson applied to memory: don't engineer features, don't hand-craft rewards, don't encode human judgment about what makes a trace "good." Give the system a scalar signal and let it figure out the rest.

Accumulated experience becomes accumulated value. Accumulated value shapes retrieval. Retrieval shapes generation. Generation shapes what traces get stored next. The loop is real. Whether it leads somewhere interesting, that's what experiments are for.
